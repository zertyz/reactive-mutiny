...
mod tests {
    ...
    async fn doc_tests() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let multi: Arc<Multi<String, 1024, 3, true, true>> = MultiBuilder::new("doc_test() event")
    ...
    async fn simple_pipelines() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let multi: Arc<Multi<u32, 1024, 2, true, true>> = MultiBuilder::new("Simple Event")
            // #1 -- event pipeline
    ...
    async fn delete_pipelines() {
        ...
        let multi_builder = MultiBuilder::new("Event with come and go pipelines");

        // correct creating & cancelling executors
        multi_builder.spawn_non_futures_non_fallible_executor(1, PIPELINE_1, |s| s, |_| async {}).await
            .expect("Single instance of PIPELINE_1 should have been created");
        multi_builder.spawn_non_futures_non_fallible_executor(1, PIPELINE_2, |s| s, |_| async {}).await
            .expect("Single instance of PIPELINE_2 should have been created");
        assert!(multi_builder.flush_and_cancel_executor(PIPELINE_1, TIMEOUT).await, "'{}' was spawned, therefore it should have been cancelled", PIPELINE_1);
        assert!(multi_builder.flush_and_cancel_executor(PIPELINE_2, TIMEOUT).await, "'{}' was spawned, therefore it should have been cancelled", PIPELINE_2);

        // attempt to double create -- which is an error condition
        multi_builder.spawn_non_futures_non_fallible_executor(1, PIPELINE_1, |s| s, |_| async {}).await
            .expect("Single instance of PIPELINE_1 should have been created");
        let result = multi_builder.spawn_non_futures_non_fallible_executor(1, PIPELINE_1, |s| s, |_| async {}).await;
        assert!(result.is_err(), "Second attempt to insert PIPELINE_1 should not have failed");

        // attempt to double cancel
        assert!(multi_builder.flush_and_cancel_executor(PIPELINE_1, TIMEOUT).await, "'{}' was spawned once, therefore it should have been cancelled", PIPELINE_1);
        assert!(!multi_builder.flush_and_cancel_executor(PIPELINE_1, TIMEOUT).await, "A pipeline cannot be cancelled twice");

        // attempt to cancel non-existing
        assert!(!multi_builder.flush_and_cancel_executor(PIPELINE_2, TIMEOUT).await, "An unexisting pipeline cannot be reported as having been cancelled");
        ...
        multi_builder.spawn_non_futures_non_fallible_executor(1, PIPELINE_2,
                                                              |s| s.inspect(move |n| *last_message_ref.try_lock().unwrap() = *(n as &u32)),
                                                              |_| async {} ).await
            .expect("Single instance of PIPELINE_2 should have been created");

        let multi: Arc<Multi<u32, 1024, 2, true, true>> = multi_builder.handle();
    ...
    async fn async_elements() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let multi: Arc<Multi<u32, 1024, 2, true, true>> = MultiBuilder::new("async event")
    ...
    async fn stats() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let multi_builder: Arc<MultiBuilder<String, 256, N_PIPELINES, true, true>> = MultiBuilder::new(event_name);
    ...
    async fn demux() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let six_multi: Arc<Multi<bool, 1024, 2, true, true>> = MultiBuilder::new("SIX")
            .spawn_non_futures_non_fallible_executor(1, "Pipeline #1", on_six_event, |_| async {}).await?
            .spawn_non_futures_non_fallible_executor(1, "Pipeline #2", on_six_event, |_| async {}).await?
            .handle();
        ...
        let two_multi: Arc<Multi<u32, 1024, 2, true, true>> = MultiBuilder::new("TWO")
        ...
        let four_multi: Arc<Multi<u32, 1024, 2, true, true>> = MultiBuilder::new("FOUR")
    ...
    async fn error_handling() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let multi: Arc<Multi<u32, 1024, 2, true, true>> = MultiBuilder::new("Event with error handling")
            .spawn_executor(1,
    ...
    async fn undegradable_latencies() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let simple_multi: Arc<Multi<SystemTime, 1024, 1, true, true>> = MultiBuilder::new("SIMPLE")
        ...
        let bloated_multi: Arc<MultiBuilder<SystemTime, 16, BLOATED_PIPELINES_COUNT, true, true>> = MultiBuilder::new("BLOATED");
...
// calm down (in debug mode). Is there a better way to resolve the syncing issue without having a huge number of multis to waste CPU?
    ...
    async fn performance_measurements() -> Result<(), Box<dyn std::error::Error>> {
        ...
        async fn profile_multi<const BUFFER_SIZE: usize,
                               ...
                               const LOG:         bool,
                               const METRICS:     bool>(multi: Arc<Multi<u32, BUFFER_SIZE, MAX_STREAMS, LOG, METRICS>>, profiling_name: &str, count: u32) {
        ...
        let multi: Arc<Multi<u32, 10240, 1, false, true>> = MultiBuilder::new(profiling_name.trim())
        ...
        let multi: Arc<Multi<u32, 10240, 1, false, false>> = MultiBuilder::new(profiling_name.trim())
            .spawn_non_futures_non_fallible_executor(1, "", |stream: MutinyStream<u32>| stream, |_| async {}).await?
            .handle();
        profile_multi(multi, profiling_name, 1024*FACTOR).await;
